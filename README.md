...
2. Главная страница (URL: `/` или `/movies`):
    ...
    - Выбранные страница списка и поисковый запрос сохраняются в sessionStorage и будут снова отображены при перезагрузке вкладки или переходе с помощью кнопок браузера
